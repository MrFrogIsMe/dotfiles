...
# installs a .tar.gz release into $OPT_DIR/<name>, stripping the top-level wrapper dir
install_from_tarball() {
  ...
  mkdir -p "$dest"
  tar -xzf "$tmp/archive.tar.gz" -C "$dest" --strip-components=1
  rm -rf "$tmp"
}
...
case "$os-$arch" in
  ...
  Linux-aarch64|Linux-arm64)
    ...
    ;;
  *)
    echo "devtools: unsupported platform $os-$arch, skipping nvim/fd/rg install" >&2
    exit 0
    ;;
esac

install_from_tarball nvim "$(gh_latest_asset_url neovim/neovim "$nvim_pat")"
install_from_tarball fd   "$(gh_latest_asset_url sharkdp/fd "$fd_pat")"
install_from_tarball rg   "$(gh_latest_asset_url BurntSushi/ripgrep "$rg_pat")"
